...
def get_args():
    ...
    parser.add_argument('--train_path', metavar='DATA_PATH', type=str, default='../NewAnno/list_bbx_image_attr_train.txt',
...
def train_val(model, train_loader, test_train_loader, val_loader, print_freq=50, optimizer=None, epoches=50, Branch=1):
    ...
    if Branch==1:
        ...
        for i in range(epoches):
            ...
            model.train()
            print("Epoch: ", i, "lr is: {}".format(StepLr.get_lr()))
            num_batches = mix_multi_task_train_epoch(model, Each_Attr_id, num_batches, train_loader, print_freq=print_freq, optimizer=optimizer)
            filename = "{}_{}_{}_Epoch.pth".format(args.arch, args.batch_size, str(i))
            torch.save(model.state_dict(), osp.join(args.savePath, filename))
        print("Finished training.")
...
def main():
    ...
    if not args.zeroTrain:
        ...
        train_loader = dataset.train_loader(args.train_path, args.relative_path, batch_size=args.batch_size, num_workers=10, pin_memory=True)
